 ...
 <div class="widget">
  ...
  <?php } ?>
      <?php if($_GET['type']=='san-pham'){?>  
        ...
        <div class="formRight">     
                <?php
          $arr_id_danhmuc = explode(',', $item['loc1']);
          $d->reset();
          $sql="select id,ten from table_news_danhmuc where type='loc-san-pham' order by stt, id asc";
          $d->query($sql);
          ...
          if(!empty($arr_danhmuc)){ ?>
                ...
                <option value="<?=$danhmuc_dm['id']?>" data-list="true" <?php if(in_array($danhmuc_dm['id'], $arr_id_danhmuc)) { echo 'selected'; } ?>><b><?=$danhmuc_dm['ten']?></b></option>
                  <?php
                  $arr_id_list = explode(',', $item['loc2']);
                  $d->reset();
                  $sql="select id,ten from table_news where type='loc-san-pham' and id_danhmuc='".$danhmuc_dm["id"]."'
                   order by stt, id desc";
                  $d->query($sql);
                  $arr_list = $d->result_array();
                  if(!empty($arr_list)){
                    foreach($arr_list as $list_dm){ ?>
                      <option value="<?=$danhmuc_dm['id']?>_<?=$list_dm['id']?>" data-cat="true" <?php 
                      if(in_array($list_dm['id'], $arr_id_list)) { echo 'selected'; } ?>><?=$list_dm['ten']?></option>
                    <?php
                    }
                  }
                  ?>
